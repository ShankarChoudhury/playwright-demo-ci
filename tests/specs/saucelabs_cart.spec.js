...
test('Sauce Labs login, verify product, and logout', async ({ page }) => {
 ...
 // await loginPage.login(process.env.USER_NAME, process.env.PASSWORD);
  ...
  inventoryPage.addSauceLabBackPackToCart();
  const price_out_cart = await inventoryPage.getcartPrice();
  console.log('Cart_Price:', price_out_cart);
  inventoryPage.clearCart();
